...
namespace Auction_Prop_API.Controllers.DBControllers
{
    public class GuarinteesController : ApiController
    {
        ...
        public async Task<IHttpActionResult> PostGuarintee(Guarintee guarintee)
        {
            ...
            {
                if (GuarinteeExists(guarintee.GuarinteeID))
                {
                    return Conflict();
                }
                else
            ...
            return CreatedAtRoute("DefaultApi", new { id = guarintee.GuarinteeID }, guarintee);
        }
        ...
        {
            return db.Guarintees.Count(e => e.GuarinteeID == id) > 0;
        }
    }
}